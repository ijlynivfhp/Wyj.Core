...
namespace Wyj.Core.Gateway.Extensions
{
    public static class CustomSwaggerSetup
    {
        public static void AddCustomSwaggerSetup(this IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                ...
                var xmlPath = Path.Combine(basePath, "Wyj.Core.Gateway.xml");
        ...
        public static void UseCustomSwaggerMildd(this IApplicationBuilder app)
        {
            ...
            {
                options.SwaggerEndpoint($"/swagger/v1/swagger.json", $"Wyj.Core.Gateway-v1");

                apis.ForEach(m =>
                {
                    options.SwaggerEndpoint($"/swagger/apiswg/{m}/swagger.json", m);
                    options.IndexStream = () => app.GetType().GetTypeInfo().Assembly.GetManifestResourceStream("Wyj.Core.ApiGateway.index.html");
                });
